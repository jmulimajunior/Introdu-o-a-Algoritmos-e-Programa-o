...
BibliotecaMunicipal/
├── README.md
├── executar.bat      # script Windows: compila e executa automaticamente
└── src/
...
### Opção rápida (Windows): script `executar.bat`

Dentro da pasta `BibliotecaMunicipal`, basta dar **duplo-clique** no ficheiro
`executar.bat` (ou executá-lo a partir do `cmd`/PowerShell). O script
compila o código-fonte e arranca o programa automaticamente, já com a
codificação UTF-8 configurada para os acentos aparecerem correctamente.

```bat
executar.bat
```

### Opção manual (Windows, Mac ou Linux)
